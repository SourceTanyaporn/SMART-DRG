...
export function ProductDraftPage({ type }) {
  ...
  return (
    <div className="space-y-5">
      <section className="overflow-hidden rounded-2xl border border-primary/15 bg-linear-to-br from-primary/10 via-white to-sky-50 p-6">
        <p className="text-xs font-semibold tracking-wider text-primary uppercase">{content.eyebrow}</p>
        <div className="mt-3 flex items-start gap-4">
          <div className="grid size-12 shrink-0 place-items-center rounded-xl bg-primary text-white shadow-lg shadow-primary/20"><Icon className="size-6" /></div>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-2 text-xs font-medium text-foreground">
          {content.steps.map((step, index) => <span key={step} className="flex items-center gap-2">{index > 0 && <ArrowRightIcon className="size-3 text-primary" />}<span className="rounded-full border border-primary/15 bg-white/80 px-3 py-1.5">{step}</span></span>)}
        </div>
      </section>
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-xl border border-[#edf0f5] bg-card p-5">
        ...
        <div className="mt-3 flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-xl font-bold text-foreground">Coding Review <span className="ml-2 rounded-full bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-600">รอตัดสินใจ</span></h1><p className="mt-1 text-sm text-muted-foreground">นายสมชาย ใจดี · AN 670520-00123 · DRG ปัจจุบัน I70Z</p></div><p className="rounded-lg bg-primary/10 px-3 py-2 text-sm font-semibold text-primary">3 ข้อเสนอแนะรอตรวจ</p></div>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_20rem]">
        <Card className="rounded-xl border border-[#edf0f5] p-5 shadow-none">
          ...
          <div className="mt-4 overflow-hidden rounded-xl border border-[#edf0f5]">
            ...
            <CodingRow code="D62" diagnosis="Acute posthemorrhagic anemia" reason="ไม่มีหลักฐานใน discharge summary" confidence="71%" decision="Reject" decisionClass="border border-rose-200 bg-rose-50 text-rose-600" />
          </div>
          <section className="mt-5 rounded-xl border border-primary/15 bg-primary/[0.025] p-4"><div className="flex items-center gap-2"><FileSearchIcon className="size-4 text-primary" /><h3 className="text-sm font-semibold text-foreground">กำลังตรวจ: E66.01 Morbid obesity</h3></div><p className="mt-2 text-sm leading-6 text-foreground">พบ BMI 38.4 และบันทึกใน pre-operative assessment ว่า obesity มีผลต่อการประเมินก่อนผ่าตัด</p><div className="mt-3 flex flex-wrap gap-2"><span className="rounded-md bg-white px-2.5 py-1.5 text-xs font-medium text-primary ring-1 ring-primary/15">ดูหลักฐานจาก Case Review</span><span className="rounded-md bg-white px-2.5 py-1.5 text-xs font-medium text-muted-foreground ring-1 ring-border">เพิ่มเหตุผลของ coder</span></div></section>
        </Card>

        <div className="space-y-4"><Card className="rounded-xl border border-[#edf0f5] p-5 shadow-none"><h2 className="text-base font-semibold text-foreground">ผลกระทบหลังยืนยัน</h2><div className="mt-4 space-y-3"><ImpactItem label="DRG" before="I70Z" after="I02Z" /><ImpactItem label="AdjRW" before="2.2234" after="2.7890" /><ImpactItem label="ค่าชดเชยโดยประมาณ" before="68,450 บาท" after="108,720 บาท" /></div><div className="mt-4 rounded-lg bg-emerald-50 p-3 text-center text-emerald-700"><p className="text-xs">เพิ่มขึ้นโดยประมาณ</p><p className="mt-1 text-xl font-bold">+40,270 บาท</p></div></Card><Card className="rounded-xl border border-violet-200 bg-violet-50/30 p-5 shadow-none"><h2 className="text-sm font-semibold text-violet-700">ก่อนส่งผลตรวจ</h2><ul className="mt-3 space-y-2 text-xs text-foreground"><li className="flex gap-2"><CheckIcon className="size-4 text-emerald-500" />Coder ตรวจครบ 3 ข้อเสนอ</li><li className="flex gap-2"><CheckIcon className="size-4 text-emerald-500" />แนบเหตุผลสำหรับรายการที่ reject</li><li className="flex gap-2"><span className="mt-1 size-2 rounded-full bg-amber-500" />รอเลือกผลสำหรับ E66.01</li></ul><button type="button" className="mt-4 h-10 w-full rounded-lg bg-primary text-sm font-semibold text-white">ยืนยัน Coding Review</button></Card></div>
      </section>
    </div>
  )
}
    ...
    <div className="space-y-4">
      <section className="rounded-xl border border-[#edf0f5] bg-card p-5">
        ...
        <div className="mt-3 flex flex-wrap items-start justify-between gap-4">
          ...
          <span className="rounded-lg bg-rose-50 px-3 py-1.5 text-sm font-semibold text-rose-600">ความเสี่ยงสูง · 3 Alerts</span>
        </div>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[13rem_minmax(0,1fr)_18rem]">
        ...
        <Card className="rounded-xl border border-[#edf0f5] p-5 shadow-none">
          <div className="flex items-center justify-between gap-3"><div><h2 className="text-sm font-semibold text-foreground">2. อ่านหลักฐานในเวชระเบียน</h2><p className="mt-1 text-xs text-muted-foreground">Progress note · 17 พ.ค. 2567 · 08:45</p></div><span className="rounded-md bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-600">AI พบ 2 จุด</span></div>
          <div className="mt-5 space-y-4 text-sm leading-7 text-foreground">
            <p>ผู้ป่วยมีอาการปวดสะโพกขวาเรื้อรัง วินิจฉัย <mark className="rounded bg-primary/15 px-1 font-semibold text-primary">osteoarthritis of hip</mark> และรับไว้รักษาเพื่อผ่าตัด</p>
            <p>มีประวัติ <mark className="rounded bg-amber-100 px-1 font-semibold text-amber-700">hypertension รับประทานยาอย่างต่อเนื่อง</mark> โดยแพทย์เฝ้าระวังความดันระหว่างนอนโรงพยาบาล</p>
            <p>วันที่ 17 พ.ค. ทำ <mark className="rounded bg-primary/15 px-1 font-semibold text-primary">total hip replacement</mark> สำเร็จ ไม่มีภาวะแทรกซ้อน</p>
          </div>
          <div className="mt-5 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-800"><b>Alert A1:</b> พบหลักฐาน hypertension ใน progress note แล้ว — ตรวจต่อว่ามีการบันทึกใน discharge summary หรือไม่ ก่อนยืนยันเป็น secondary diagnosis</div>
        </Card>
...
function FlowStep({ number, title, detail, done, active }) {
  return <div className={`rounded-lg border p-3 ${active ? "border-primary bg-white shadow-sm" : "border-primary/10 bg-white/70"}`}><div className="flex items-center gap-2"><span className={`grid size-5 place-items-center rounded-full text-xs font-bold ${done ? "bg-emerald-500 text-white" : active ? "bg-primary text-white" : "bg-muted text-muted-foreground"}`}>{done ? <CheckIcon className="size-3" /> : number}</span><p className="text-sm font-semibold text-foreground">{title}</p></div><p className="mt-1 pl-7 text-xs text-muted-foreground">{detail}</p></div>
}
